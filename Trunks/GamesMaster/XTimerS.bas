...
' maxti is an array of active GTimer objects.  The reason
' -----   for using an array of user-defined types
'   instead of a Collection object is to get early
'   binding when we raise the GTimer object's Tick event.
...
' BeginTimer function is called by an GTimer object when
' -------------------   the GTimer's Interval property is
...
'   function puts a reference to the GTimer object into
'   the array maxti.  This reference will be used to call
'   the method that raises the GTimer's Tick event.
'
Public Function BeginTimer(ByVal xt As GTimer, ByVal Interval As Long)
...
Public Sub TimerProc(ByVal hWnd As Long, ByVal uMsg As Long, ByVal idEvent As Long, ByVal lngSysTime As Long)
    ...
    For intCt = 1 To mintMaxTimers
        If maxti(intCt).id = idEvent Then
            ...
            If Err.Number <> 0 Then
                ' If an error occurs, the GTimer has
    ...
    '   GTimer somehow got freed without the Windows
...
' EndTimer procedure is called by the GTimer whenever
...
'
Public Sub EndTimer(ByVal xt As GTimer)
    Dim lngTimerID As Long
    Dim intCt As Integer
